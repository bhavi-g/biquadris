...
// Default constructor
JBlock::JBlock(int level) : Block(level), rotationState(0) {
    // Initialize in the default orientation at the bottom-left corner
    structure = {
        Cell(0, 2, 'J'), 
        Cell(0, 3, 'J'),Cell(1, 3, 'J'), Cell(2, 3, 'J')
    };
...
JBlock::JBlock(bool levelHeavy, int level) : Block(levelHeavy, level), rotationState(0) {
    structure = {
        Cell(0, 2, 'J'), 
        Cell(0, 3, 'J'),Cell(1, 3, 'J'), Cell(2, 3, 'J')
    };
...
// Clone the block
std::shared_ptr<Block>JBlock::cloneBlock() const {
    return std::make_shared<JBlock>(*this);
}
